...
class DenoiseUNet(nn.Module):
    ...
    def q_sample(self, x_start, t, noise):
        ...
        mean = sqrt_alphas_cumprod_t * x_start  # compute the mean of q(x_t | x_0), hint: use self.sqrt_alphas_cumprod
        std = sqrt_one_minus_alphas_cumprod_t  # compute the standard deviation term, hint: use self.sqrt_one_minus_alphas_cumprod
    ...
    def forward(self, batch):
        ...
        if t is None:
            t = torch.randint(0, self.timesteps, (batch_size,), device=x0.device) # draw random timesteps for the batch
        noise = batch.get("noise")
        if noise is None:
            noise = torch.randn_like(x0)  # draw Gaussian noise for the forward process
        xt = self.q_sample(x0, t, noise)  # sample from q(x_t | x_0)
        time_emb = self.time_embedding(t)
        h0 = self.model["init"](xt) # apply the initial convolution
        skip0, h1 = self.model["down0"](h0, time_emb) # apply the first down block
