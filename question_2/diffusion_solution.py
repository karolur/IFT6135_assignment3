...
def q_sample(x_start, t, coefficients, noise=None):
    ...
    sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod = coefficients # these are values for the whole batch
...
def p_sample(model, x, t, t_index, coefficients, noise=None):
    ...
    with torch.no_grad():
        ...
        betas, sqrt_one_minus_alphas_cumprod, sqrt_recip_alphas, posterior_variance = (
            coefficients # these are all size of the timesteps and they are all sorted by timestep
        )
        beta_t = extract(betas, t, x.shape) # this is batchsize
        alpha_t = 1.0 - beta_t # these are all the alfa_t
        sqrt_one_minus_alphas_cumprod_t = extract(sqrt_one_minus_alphas_cumprod, t, x.shape) # sqrt(1-prod(alfas))
        sqrt_recip_alphas_t = extract(sqrt_recip_alphas, t, x.shape) # 1/sqrt(alfa_t)
        posterior_variance_t = extract(posterior_variance, t, x.shape)

        p_mean = sqrt_recip_alphas_t * (x - ((1-alpha_t) / sqrt_one_minus_alphas_cumprod_t) * predicted_noise)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(
            ...
            if noise is None:
                ...
                img = p_sample(model, img, t, i, coefficients, noise[i].to(model.device))
            imgs.append(img.cpu())
