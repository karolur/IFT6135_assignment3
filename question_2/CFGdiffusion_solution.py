...
def p_sample(model, x, t, t_index, coefficients, y=None, guidance_scale=1.0, noise=None):
    ...
    eps_uncond = model(x, t, y=None) # # unconditional prediction
    if y is None or guidance_scale == 0.0:
        eps_theta = eps_uncond
    else:
        eps_cond = model(x, t, y) # conditional prediction
    ...
    p_mean = sqrt_recip_alphas_t * (x - (beta_t / sqrt_one_minus_alphas_cumprod_t) * eps_theta)
...
def p_sample_loop(model, shape, timesteps, T, coefficients, y=None, guidance_scale=1.0, noise=None):
    ...
    for i in tqdm(
        ...
        if noise is None:
            img = p_sample(model, img, t, i, coefficients, y, guidance_scale, noise)
        else:
            img = p_sample(model, img, t, i, coefficients, y, guidance_scale, noise[i])
        imgs.append(img.cpu())
...
def p_losses(denoise_model, x_start, t, coefficients, y=None, p_uncond=0.1, noise=None, loss_type="l2"):
    ...
    if y is not None:
        ...
        mask = (torch.rand(t.shape, device=t.device) < p_uncond)
    ...
    if y is None:
        ...
        eps_theta = torch.where(mask.view(-1, 1, 1, 1), eps_uncond, eps_cond)
